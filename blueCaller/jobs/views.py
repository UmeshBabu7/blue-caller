...
class WorkerListView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        for worker in workers:
            ...
            worker.full_stars = [1] * full_stars
            worker.half_star = [1] * half_star
            worker.empty_stars = [1] * empty_stars
...
class WorkerDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Add to context under worker object
        context['worker'] = {
            'average_rating': average_rating,
            'total_ratings': total_ratings,
            'full_stars': [1] * full_stars,
            'half_star': [1] * half_star,
...
def delete_appointment(request, appointment_id):
    ...
    if request.user == appointment.customer.owner or request.user == appointment.worker.owner:
        ...
        if request.user == appointment.customer.owner:
            ...
            return redirect('worker_appointments') # Worker view for their appointments
    else:
        messages.error(request, "You are not authorized to delete this appointment.")
        
    return redirect('worker-list') # Fallback redirect
